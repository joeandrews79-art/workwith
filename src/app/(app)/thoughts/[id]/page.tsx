...
export default async function ThoughtDetailPage({
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <Link href="/thoughts" className="text-sm text-muted hover:text-ink">← Thoughts</Link>

      <header className="card p-5">
        <p className="text-xs text-faint mb-1">Captured {formatDate(t.createdAt)}</p>
        <h1 className="text-xl font-bold tracking-tight">{t.text}</h1>
        {t.detail && <p className="text-ink-soft mt-2 whitespace-pre-wrap">{t.detail}</p>}
